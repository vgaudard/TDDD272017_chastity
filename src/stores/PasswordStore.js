    ...
    reduce(state: State, action: Action): State {
        switch (action.type) {
            ...
            case 'edit/set-url':
                return state.update(
                    action.id,
                    (lo : LoadObject) => lo.updating().map(password => {
                        return password.set('url', action.value);
                    })
                );


            case 'edit/set-username':
                return state.update(
                    action.id,
                    (lo : LoadObject) => lo.updating().map(password => {
                        return password.set('username', action.value);
                    })
                );


            case 'edit/set-password':
                return state.update(
                    action.id,
                    (lo : LoadObject) => lo.updating().map(password => {
                        return password.set('password', action.value);
                    })
                );
            ...
            case 'edit/set-notes':
                return state.update(
                    action.id,
                    (lo : LoadObject) => lo.updating().map(password => {
                        return password.set('notes', action.value);
                    })
                );

            case 'edit/finish-start':
                PasswordDataManager.updatePassword(action._id, action.url, action.username, action.password, action.notes);
                return state;
